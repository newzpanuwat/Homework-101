...
public class Report extends JFrame implements ActionListener  {
	...
	public Report(){
		...
		/*JLabel lb_marid = new JLabel("รหัสตลาดนัด(Market ID)");
		txt_marid = new JTextField();
		PN1.add(lb_marid);
		PN1.add(txt_marid);
		
		JLabel lb_marna = new JLabel("ชื่อตลาดนัด(Market Name)");
		txt_marna = new JTextField();
		PN1.add(lb_marna);
		PN1.add(txt_marna);
		
		JLabel lb_marlo = new JLabel("ที่ตั้งตลาดนัด(Market Location)");
	...
	String columns[]={"รหัสผู้ขาย","ชื่อผู้ขาย","เบอร์โทร","จำนวนแผง"};
	...
	void showData(){
		try{
			...
			String sql = "SELECT * FROM SELLER"
					+ " WHERE SL_ID LIKE '%"+ search + "%'"
					+ " OR SL_NAME LIKE '%"+ search + "%'"
					+ " OR SL_TEL LIKE '%"+ search + "%'"
					+ " OR SL_NUM LIKE '%"+ search + "%'";
			...
			//4.Looping resultSet
					...
					while(RS.next())
					{
						...
						modelComp.setValueAt(RS.getString("SL_ID"),row, 0);
						modelComp.setValueAt(RS.getString("SL_NAME"),row, 1);
						modelComp.setValueAt(RS.getString("SL_TEL"),row, 2);
						modelComp.setValueAt(RS.getString("SL_NUM"),row, 3);
						row++;
					}
...
public void Delete()
{
	...
	try{
		...
		String sql = 	"DELETE FROM SELLER WHERE SL_ID=? " ;
	...
	public static void main(String[] args) {
		...
		report.setTitle("แสดงรายงาน");
		report.setSize(800, 500);
