...
public class Market extends JFrame implements ActionListener  {
	...
	String columns[]={"รหัสตลาดนัด","ชื่อตลาดนัด","ที่ตั้งตลาดนัด"};
	...
	void showData(){
		try{
			...
			String sql = "SELECT * FROM MARKET"
					+ " WHERE MA_ID LIKE '%"+ search + "%'"
					+ " OR MA_NAME LIKE '%"+ search + "%'"
					+ " OR MA_ADDRESS LIKE '%"+ search + "%'";
			...
			//4.Looping resultSet
					...
					while(RS.next())
					{
						...
						modelComp.setValueAt(RS.getString("MA_ID"),row, 0);
						modelComp.setValueAt(RS.getString("MA_NAME"),row, 1);
						modelComp.setValueAt(RS.getString("MA_ADDRESS"),row, 2);
						row++;
					}
...
public void Delete()
{
	...
	try{
		...
		String sql = 	"DELETE FROM MARKET WHERE MA_ID=? " ;
